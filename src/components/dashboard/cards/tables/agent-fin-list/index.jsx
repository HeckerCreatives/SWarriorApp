...
import AgentFinListTableRow from "./row";

// ** Style
import "./index.css";
import useAgentStore from "../../../../../stores/agentStore";

const AgentFinListTable = () => {
  const limit = 10;
  const [page, setPage] = useState(1);
  const [filter, setFilter] = useState("");

  const getAgents = useAgentStore(state => state.getFinancers);
  const search = useAgentStore(state => state.searchFinancers);
  const agents = useAgentStore(state => state.financer.agents);
  const totalPages = useAgentStore(state => state.financer.totalPages);
  const nextPage = useAgentStore(state => state.financer.nextPage);
  const prevPage = useAgentStore(state => state.financer.prevPage);
  const loading = useAgentStore(state => state.loading.financer);
          ...
          <table className="afl-table h-100">
            ...
            <tbody>
              {loading ? (
                ...
                agents.map(agent => (
                  <AgentFinListTableRow key={agent._id} agent={agent} />
                ))
              )}
            </tbody>
          </table>
...
export default AgentFinListTable;
