...
const ModAcctListTable = () => {
  ...
  const getAgents = useAgentStore(state => state.getAccountants);
  const search = useAgentStore(state => state.searchAccountants);
  const agents = useAgentStore(state => state.accountant.agents);
  const totalPages = useAgentStore(state => state.accountant.totalPages);
  const nextPage = useAgentStore(state => state.accountant.nextPage);
  const prevPage = useAgentStore(state => state.accountant.prevPage);
  const loading = useAgentStore(state => state.loading.accountant);
          ...
          <table className="mul-table h-100">
            ...
            <tbody>
              {loading ? (
                ...
                agents.map(agent => (
                  <ModAcctListTableRow key={agent._id} agent={agent} />
                ))
              )}
            </tbody>
          </table>
